...
function CheckoutContent() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <div className="text-center bg-white rounded-2xl border border-slate-200 p-8 max-w-sm">
          ...
          <Link href="/login" className="block w-full bg-blue-600 text-white font-semibold py-3 rounded-xl hover:bg-blue-700 transition">
            Fazer login
          </Link>
        </div>
      </div>
  ...
  async function handleCheckout() {
    ...
    try {
      ...
      if (!res.ok) {
        setError((data as unknown as { error: string }).error || 'Erro ao processar pagamento');
      } else {
        setResult(data);
      }
    } catch {
        ...
        {!result ? (
          <>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-6 mb-6">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <label key={opt.id} className={`flex items-center gap-3 p-4 rounded-xl border cursor-pointer transition ${method === opt.id ? 'border-blue-500 bg-blue-50' : 'border-slate-200 hover:border-slate-300'}`}>
                    <input type="radio" name="method" value={opt.id} checked={method === opt.id} onChange={() => setMethod(opt.id as PaymentMethod)} className="accent-blue-600" />
            ...
            <button
              ...
              disabled={loading}
              className="w-full bg-gradient-to-r from-blue-600 to-indigo-500 text-white font-bold py-4 rounded-xl hover:opacity-90 transition disabled:opacity-50"
            >
          ...
          <div className="space-y-6">
            ...
            {result.invoiceUrl ? (
              <a href={result.invoiceUrl} target="_blank" rel="noopener noreferrer" className="block text-center text-blue-600 hover:underline text-sm">
                Ver fatura no Asaas →
              </a>
            ) : null}
